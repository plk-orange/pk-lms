                ...
                <div className={playerStyles.cont}>
                    <div className={playerStyles.title}>
                        <div className={playerStyles.name}>
                            <Moment format='MM/DD/YYYY'>{player.match}</Moment>
                        </div>

                        <p className={playerStyles.team}>
                            <Link to={`team/${player.team_name}`}>
                                {player.team_name}{' '}
                            </Link>
                        </p>
                    </div>
            ...
            <div className='divide-y-2'>
                {players &&
                    players.map((player, i) => (
                        <PlayerCardItem key={i} player={player} />
                    ))}
            </div>
        </>
    )
...
const playerStyles = {
    wrap: ` border border-pkgray-md opacity-75 border-y-0 hover:bg-pkgray-xxl transition duration-200 ease-out `,
